...
sh
/usr/sbin/unmount -o immediate /u/${instance-EKMF_WEB_TASK_USER};
if [ $? -gt 0 ]; then 
    echo "no web task user directory needed to be unmounted";
fi;
/usr/sbin/unmount -o immediate /u/${instance-EKMF_AGENT_TASK_USER};
if [ $? -gt 0 ]; then 
...
//SYSIN    DD *,SYMBOLS=JCLONLY
 DELETE OMVS.MV3N.USERID.${instance-EKMF_WEB_TASK_USER}
 IF MAXCC EQ 8 THEN DO
   SET MAXCC = 0
   END
 DELETE OMVS.MV3N.USERID.${instance-EKMF_AGENT_TASK_USER}
 IF MAXCC EQ 8 THEN DO
   SET MAXCC = 0
   END
/*
...
SH
rm -rf /u/${instance-EKMF_WEB_TASK_USER};
if [ $? -gt 0 ]; then 
    echo "no web task user directory needed to be deleted";
fi;
rm -rf /u/${instance-EKMF_AGENT_TASK_USER};
if [ $? -gt 0 ]; then 
